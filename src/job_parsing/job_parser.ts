...
if (JobParser.job_parent_regex.exec(lineText)) {
return lineText.replace(/\s/g, "").toLowerCase().split(":").pop();
}
return undefined;
}
...
const line = document.lineAt(job_line_number);
const lineText = line.text;
if (JobParser.job_name_regex.exec(lineText)) {
return lineText.replace(/\s/g, "").toLowerCase().split(":").pop();
}
return undefined;
}
...
const line = document.lineAt(job_line_number);
const lineText = line.text;
if (JobParser.playbook_path_regex.exec(lineText)) {
const splitLine = lineText.replace(/\s/g, "").toLowerCase().split(":");
if (splitLine.length === 1) {
return splitLine.pop()?.substr(1); // Remove the '-' from the beginning of the string
} else {
...
let lineNumberIterator = job_line_number;

// From the current line, search downwards.
while (true) {
const job_attribute = JobParser.parse_job_attribute_from_line(lineNumberIterator, document);
if (job_attribute) {
...
lineNumberIterator++;
if (JobParser.at_the_end_of_job_definition(document, lineNumberIterator)) {
break;
}
}

lineNumberIterator = job_line_number;
...
if (JobParser.job_regex.exec(lineText)) {
return true;
}
return false;
}

static parse_ansible_variable_from_position_in_line(
document: vscode.TextDocument,
...
const startPosition = job_line.range.start.translate({ characterDelta: match.index - 1 });
const endPosition = startPosition.translate({ characterDelta: match[1].length + 2 });
