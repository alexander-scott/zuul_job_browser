...
if (parent_name) {
const parent_job = this.jobManager.get_job_with_name(parent_name);
if (parent_job) {
...
return new vscode.Location(job.document, jobNameLocation.get_as_vscode_location());
}
}
...
// Jump to ansible variable definition
const ansible_var = JobParser.parse_ansible_variable_from_position_in_line(document, position);
if (ansible_var) {
const ansibleJobName = JobParser.parse_job_from_random_line_number(document, position.line);
if (ansibleJobName) {
const job = this.jobManager.get_job_with_name(ansibleJobName);
if (job) {
const attributes = JobAttributeCollector.get_attributes_for_job(job, this.jobManager);
const selected_attribute = JobAttributeCollector.get_specific_attribute_from_array(attributes, ansible_var);
if (selected_attribute) {
const attribute_owner = this.jobManager.get_job_with_name(selected_attribute.job_name);
if (attribute_owner) {
