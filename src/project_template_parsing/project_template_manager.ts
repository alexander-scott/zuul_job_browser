...
export class ProjectTemplateManager {
	...
	get_single_job_on_line(document: vscode.Uri, line_number: number): Location | undefined {
		...
		for (const template of valid_templates) {
			const job_on_line = template.get_all_value_locations().find((loc) => loc.line_number === line_number);
			if (job_on_line) {
				return job_on_line;
			}
		}
	...
	get_first_job_with_name(job_name: string): Location | undefined {
		for (const template of this._project_templates) {
			const job = template.get_all_value_locations().find((loc) => loc.value === job_name);
			if (job) {
				return job;
			}
		}
